...
def ModExp(Folder0, FolderN, Vars, Pars):

    F0s = os.listdir(f'{Folder0}')
    F0s = [F0 for F0 in F0s if '_mF_' in F0]
    F0s.sort()
    NRuns = len(F0s)
    Nchars = int(np.log10(NRuns)) + 1
    ...
    for iRun in np.arange(NRuns):
        ...
        # Read base file
        p0 = run_path(f'{Folder0}/{F0s[iRun]}/pars.py')

        # Repeats are a parameter, but never multivalued
        repeats = Pars['repeats'] if 'repeats' in Pars else p0['repeats']

        # Determine the value of all variables
        E = drawVal(RNG, Vars['E']) if 'E' in Vars else p0['E']
        K = drawVal(RNG, Vars['K']) if 'K' in Vars else p0['K']
        h = drawVal(RNG, Vars['h']) if 'h' in Vars else p0['h']
        u = drawVal(RNG, Vars['u']) if 'u' in Vars else p0['u']
        if 'phi0' not in Vars: Vars['phi0'] = [0, 2 * np.pi, 'uniform']

        SpecType = Pars['SpecType'] if 'SpecType' in Pars else p0['SpecType']
        FracCrit = Pars['FracCrit'] if 'FracCrit' in Pars else p0['FracCrit']
        tail_fac = Pars['tail_fac'] if 'tail_fac' in Pars else [p0['tail_fac']]
        multiFrac = Pars['multiFrac'] if 'multiFrac' in Pars else [p0['multiFrac']]

        if 'SpecType' in Pars:
            if Pars['SpecType'] == p0['SpecType']:
                phi0 = p0.phi0
            elif 'Mono' in Pars['SpecType'] and 'Mono' not in p0['SpecType']:
                phi0 = p0.phi0[5, :]
            elif 'Mono' not in Pars['SpecType'] and 'Mono' in p0['SpecType']:
                phi0 = drawVal(RNG, Vars['phi0'], [21, repeats])
                if 'phi0' in p0: phi0[5, :] = p0['phi0']
        ...
        for ST in SpecType:
            for FC in FracCrit:
                for mF in multiFrac:
                    for tf in tail_fac:
                        if 'PowerLaw' in ST:
                            ExpN = f'PowerLaw{ST[1]}_{FC}_mF_{mF}_tf_{tf}'
                        else:
                            ExpN = f'{ST[0]}_{FC}_mF_{mF}_tf_{tf}'
                        if iRun == 0 and not os.path.exists(f'{FolderN}/{ExpN}'):
                            os.mkdir(f'{FolderN}/{ExpN}')

                        if not os.path.exists(f'{FolderN}/{ExpN}/{ExpN}_{RunN}'):
                            os.mkdir(f'{FolderN}/{ExpN}/{ExpN}_{RunN}')

                        with open(f'{FolderN}/{ExpN}/{ExpN}_{RunN}/pars.py', 'w') as f:
                            f.write('#!/usr/bin/env python3\n')
                            f.write('# -*- coding: utf-8 -*-\n')
                            f.write('\n')
                            f.write('import numpy as np\n')
